...
3. Tag the theme and demo with the version in their `ext_emconf.php` files (`v1.4.0` for this release) in their distribution repositories. Register the repositories on Packagist. Do not claim a new version is available there until the package pages resolve.
4. From a fresh checkout of `starter/`, run `composer install` using public package resolution. Install TYPO3 and optionally require `crispframe/agency-demo:^1.4` on a new empty site. Edit branding, contact recipients, legal pages and a German content record in the backend; test delivery and publish a page in both languages.
