...
3. Tag the theme and demo as `v1.0.0` in their distribution repositories. Keep `ext_emconf.php` versions aligned with the tags. Register the repositories on Packagist. Do not claim either package is available there until the package pages resolve.
...
The test script builds temporary archive packages with a `1.0.0` version solely to prove Composer dependency resolution before public tags exist. It does not modify the source package manifests or substitute for the final Packagist checkout.
